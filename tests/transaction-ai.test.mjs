...
const requestWith = ({ description = "", image, token = "valid-token" } = {}) => {
  ...
  form.set("currencies", JSON.stringify(categories.currencies));
...
test("accepts a description or supported image and returns a normalized review draft", async () => {
  ...
  assert.deepEqual(analyzedInput.currencies, categories.currencies);
...
test("accepts only currencies supplied by the current Moneta workspace", () => {
  const supported = normalizeTransactionAiDraft({
    date: "2026-08-19",
    type: "expense",
    category: "Food",
    description: "Paris groceries",
    amount: 18.4,
    currency: "EUR",
    countsTowardMonthlyBudget: true,
    uncertainFields: [],
  }, categories);
  assert.equal(supported.draft.currency, "EUR");
  assert.equal(supported.needsReview.includes("currency"), false);

  const unsupported = normalizeTransactionAiDraft({
    ...supported.draft,
    currency: "BTC",
    uncertainFields: [],
  }, categories);
  assert.equal(unsupported.draft.currency, null);
