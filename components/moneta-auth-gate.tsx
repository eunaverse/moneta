...
export function MonetaAuthGate({ children }: { children: (account: MonetaAccount) => ReactNode }) {
  ...
  if (!session) {
    ...
    return <main className="auth-shell"><section className="auth-card"><div className="auth-mark">M</div><span>PRIVATE MONEY WORKSPACE</span><h1>Your finances,<br /><em>available anywhere.</em></h1><p>Plan a fixed runway, record real spending, and see every assumption behind the result.</p><ul className="auth-benefits"><li>Private account-level financial data</li><li>Live budgets, schedules, and what-if planning</li><li>Portable backup whenever you need it</li></ul><button type="button" className="google-sign-in" disabled={submitting} onClick={signInWithGoogle}><svg viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M21.6 12.23c0-.71-.06-1.39-.18-2.05H12v3.87h5.38a4.6 4.6 0 0 1-2 3.02v2.51h3.24c1.9-1.75 2.98-4.33 2.98-7.35Z"/><path fill="#34A853" d="M12 22c2.7 0 4.97-.9 6.62-2.42l-3.24-2.51c-.9.6-2.05.96-3.38.96-2.61 0-4.82-1.76-5.61-4.13H3.04v2.59A10 10 0 0 0 12 22Z"/><path fill="#FBBC05" d="M6.39 13.9A6.02 6.02 0 0 1 6.08 12c0-.66.11-1.3.31-1.9V7.51H3.04A10 10 0 0 0 2 12c0 1.61.38 3.13 1.04 4.49l3.35-2.59Z"/><path fill="#EA4335" d="M12 5.97c1.47 0 2.79.51 3.83 1.5l2.87-2.88A9.63 9.63 0 0 0 12 2a10 10 0 0 0-8.96 5.51l3.35 2.59C7.18 7.73 9.39 5.97 12 5.97Z"/></svg><span>{submitting ? "Opening Google…" : "Continue with Google"}</span></button>{message && <div className="auth-message" role="alert">{message}</div>}<small>Google may show the Supabase project domain used by Moneta&apos;s secure data service.</small></section></main>;
  }
