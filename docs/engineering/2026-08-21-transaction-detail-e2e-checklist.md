...
- Long merchant, item, and category text wrap rather than clipping amounts or actions.
...
- `tests/e2e/transaction-detail.spec.ts`: detail content, reconciliation, fallback, keyboard close, Edit transition, selection isolation, and responsive overflow.
