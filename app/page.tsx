      ...
      <section className="workspace">
          ...
          <div className="transaction-layout">
            <form className="transaction-form" onSubmit={addEntry}>
              ...
              {!editingEntryId && <div className="transaction-entry-method wide" role="group" aria-label="Entry method">
                <button type="button" className={transactionEntryMode === "ai" ? "active" : ""} aria-pressed={transactionEntryMode === "ai"} onClick={() => setTransactionEntryMode("ai")}><span>✦</span><strong>Describe with AI</strong><small>Turn a description or screenshot into a draft</small></button>
                <button type="button" className={transactionEntryMode === "manual" ? "active" : ""} aria-pressed={transactionEntryMode === "manual"} onClick={() => setTransactionEntryMode("manual")}><span>＋</span><strong>Enter manually</strong><small>Fill in the transaction details yourself</small></button>
              </div>}
              {!editingEntryId && transactionEntryMode === "ai" && <section className="transaction-ai-assistant wide" aria-labelledby="transaction-ai-title">
                <div className="transaction-ai-heading"><div><span>AI-ASSISTED</span><h3 id="transaction-ai-title">{locale === "ko" ? "거래를 설명하거나 결제 화면을 올리세요" : "Describe a transaction or add a screenshot"}</h3><p>{locale === "ko" ? "Moneta가 검토 가능한 거래 초안을 만듭니다." : "Moneta creates a draft for you to review."}</p></div><i aria-hidden="true">✦</i></div>
        ...
        {view === "budget" && <div className="page">
          ...
          <section className="budget-capacity-section">
            <div className="budget-capacity-copy"><span>SAFE MONTHLY SPEND</span><h2>Available after planned bills</h2><p>Moneta reserves unpaid scheduled payments, then spreads the remaining money through your plan end date.</p><div className="capacity-plan-facts"><div><span>AVAILABLE AFTER BILLS</span><strong>{money.format(planningCapacity.availableToSpread)}</strong></div><div><span>PLAN PERIOD</span><strong>{planningPeriodLabel}</strong></div><div><span>MONTHS LEFT</span><strong>{remainingPlanningMonths} · through {data.planningEndMonth}</strong></div></div></div>
        ...
        {view === "what-if" && <div className="page what-if-page">
          ...
          <div className="what-if-layout">
            <article className="what-if-controls">
              ...
              <div className={`scenario-period ${scenarioMonths === 0 ? "ended" : ""}`}><span>PLAN END DATE</span><strong>{data.planningEndMonth}</strong><small>{scenarioMonths > 0 ? `${scenarioMonths} months remaining · ${forecastStartMonth}–${data.planningEndMonth}` : "This plan has ended. Choose a new end month in Plan setup."}</small></div>
            </article>
        ...
        {view === "settings" && <div className="page settings-page">
          <div className="page-title compact"><div><span>{locale === "ko" ? "계획의 기준" : "PLAN FOUNDATION"}</span><h1 aria-label={`${locale === "ko" ? "계획 설정" : "Plan setup"} · Settings`}>{locale === "ko" ? "계획 설정" : "Plan setup"}</h1><p>{locale === "ko" ? "Moneta의 모든 계산에 쓰이는 돈, 기간, 분류를 설정하세요." : "Set the money, dates, and categories Moneta uses for every forecast."}</p></div><span className="settings-save-note">● Changes save automatically</span></div>
          <section className="settings-section plan-foundation-section" aria-labelledby="plan-foundation-heading">
            <div className="settings-section-heading"><span>CORE PLAN INPUTS</span><h2 id="plan-foundation-heading">What Moneta calculates from</h2><p>These inputs change your safe monthly spend and plan forecasts.</p></div>
        ...
        {view === "insights" && <div className="page insights-page">
          <div className="page-title compact"><div><span>{locale === "ko" ? "기간별 신호" : "PERIOD SIGNALS"}</span><h1 aria-label={`${locale === "ko" ? "지출 분석" : "Spending insights"} · Insights`}>{locale === "ko" ? "지출 분석" : "Spending insights"}</h1></div><div className="insight-range-controls"><label className="month-picker"><span>ANALYSIS THROUGH</span><input aria-label="Analysis through month" type="month" value={selectedMonth} onChange={(event) => { chooseMonth(event.target.value); setOverLimitLimit(5); }} /></label><label className="month-picker"><span>LOOKBACK</span><div><input aria-label="Analysis lookback months" type="number" min="1" max="24" value={insightMonths || ""} onChange={(event) => { setInsightMonths(Math.max(0, Math.min(24, Number(event.target.value) || 0))); setOverLimitLimit(5); }} onBlur={() => { if (insightMonths < 1) setInsightMonths(6); }} /> months</div></label></div></div>
          {!hasInsightSpending ? <>
            <article className="insight-action-card insights-empty-state">
              ...
              <div className="insights-empty-copy"><span>START WITH ACTUAL SPENDING</span><strong>Add a transaction to unlock spending insights</strong><p>Your saved expenses will fill in the analysis preview below.</p><button type="button" onClick={goToAddTransaction}>Add first transaction</button></div>
            </article>
            <section className="insights-preview" aria-labelledby="insights-preview-title">
              <div className="insights-preview-heading"><div><span>WHAT INSIGHTS WILL SHOW</span><h2 id="insights-preview-title">Understand what changed and where your money went</h2></div><p>These fill in after your first expense.</p></div>
              <div className="insights-preview-layout">
                <article className="insights-preview-trend"><div><span>SPENDING TREND</span><strong>Spending trend</strong><small>Compare monthly expenses over your selected lookback.</small></div><div className="insights-preview-bars" aria-hidden="true"><i /><i /><i /><i /><i /><i /></div></article>
                <div className="insights-preview-signals"><article><span>TOP CATEGORY</span><strong>Top category</strong><small>See where most of your spending goes.</small></article><article><span>BUDGET ALERTS</span><strong>Budget alerts</strong><small>See which category limits are close to or over budget.</small></article></div>
              </div>
            </section>
          </> : <>
