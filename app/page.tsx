    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "overview" && <div className="page overview-page">
          ...
          <article className={`month-card overview-budget-card ${!hasCategoryBudgetData ? "empty" : ""}`}><div className="card-heading"><div><span>{selectedMonth}</span><h2>This month&apos;s category budgets</h2></div><button onClick={() => navigate("budget")}>{hasCategoryBudgetData ? "Edit budgets" : "Add budget"}</button></div>{!hasCategoryBudgetData ? <div className="overview-budget-empty"><strong>No category budget yet</strong><span>Add limits for categories such as Food or Transport when you are ready to track monthly spending.</span></div> : <><div className="budget-remaining"><span>CATEGORY BUDGET REMAINING</span><strong className={budgetAvailable < 0 ? "danger-text" : "success-text"}>{money.format(budgetAvailable)}</strong></div>{monthlyBudgetAbovePlanSafe > 0 && <p className="plan-budget-warning" role="status">Your category budget is {money.format(monthlyBudgetAbovePlanSafe)}/month above the safe monthly spend.</p>}<div className="stacked-track"><i className="actual" style={{ width: `${Math.min(100, monthlyFlexibleBudgetForSelectedMonth > 0 ? monthlyBudgetExpenseTotal / monthlyFlexibleBudgetForSelectedMonth * 100 : 0)}%` }} /></div><div className="budget-breakdown two"><div><i className="budget-dot" /><span>Monthly category limits</span><strong>{money.format(monthlyFlexibleBudgetForSelectedMonth)}</strong></div><div><i className="actual-dot" /><span>Spent from category limits</span><strong>{money.format(monthlyBudgetExpenseTotal)}</strong></div></div><p className="clarity-note">Only expenses marked for category budgets reduce this balance. Other expenses stay visible in Transactions; scheduled payments are reserved separately.</p></>}</article>
        ...
        {view === "settings" && <div className="page settings-page">
          <div className="page-title compact"><div><span>{locale === "ko" ? "MONETA 설정" : "MONETA SETTINGS"}</span><h1>{locale === "ko" ? "설정" : "Settings"}</h1><p>{locale === "ko" ? "계획에 사용되는 기준과 구조를 관리하세요." : "Manage the assumptions and structure behind your plan."}</p></div><span className="settings-save-note">● Changes save automatically</span></div>
