    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "overview" && <div className="page overview-page">
          ...
          <article className={`month-card overview-budget-card ${!hasCategoryBudgetData ? "empty" : ""}`}><div className="card-heading"><div><span>{selectedMonth}</span><h2>This month&apos;s category budgets</h2></div><button onClick={() => navigate("budget")}>{hasCategoryBudgetData ? "Edit budgets" : "Add budget"}</button></div>{!hasCategoryBudgetData ? <div className="overview-budget-empty"><strong>No category budget yet</strong><span>Add limits for categories such as Food or Transport when you are ready to track monthly spending.</span></div> : <><div className="budget-remaining"><span>CATEGORY BUDGET REMAINING</span><strong className={budgetAvailable < 0 ? "danger-text" : "success-text"}>{money.format(budgetAvailable)}</strong></div>{monthlyBudgetAbovePlanSafe > 0 && <p className="plan-budget-warning" role="status">Your category budget is {money.format(monthlyBudgetAbovePlanSafe)}/month above the safe monthly spend.</p>}<div className="stacked-track"><i className="actual" style={{ width: `${Math.min(100, monthlyFlexibleBudgetForSelectedMonth > 0 ? monthlyBudgetExpenseTotal / monthlyFlexibleBudgetForSelectedMonth * 100 : 0)}%` }} /></div><div className="budget-breakdown"><div><i className="budget-dot" /><span>Category budgets</span><strong>{money.format(monthlyFlexibleBudgetForSelectedMonth)}</strong></div><div><i className="actual-dot" /><span>Budget spending</span><strong>{money.format(monthlyBudgetExpenseTotal)}</strong></div><div><i className="scheduled-dot" /><span>Outside category budgets</span><strong>{money.format(monthlyOutsideBudgetTotal)}</strong></div></div><p className="clarity-note">Scheduled payments are reserved separately. Unlinked expenses reduce net worth as additional spending.</p></>}</article>
        ...
        {view === "insights" && <div className="page insights-page">
          ...
          {!hasInsightSpending ? <article className="insight-action-card insights-empty-state">
            ...
            <div className="insights-empty-copy"><span>START WITH ACTUAL SPENDING</span><strong>Add a transaction to unlock spending insights</strong><p>Once you record an expense, Moneta will show the trend, top category, and any category that exceeds its budget.</p><button type="button" onClick={goToAddTransaction}>Add first transaction</button></div>
          </article> : <>
